...
class PictureDownload(Thread):
    ...
    def run(self):
        ...
        picture_file = f"C:\\Users\\CBAS\\Documents\\Python Scripts\\Pycharm\\TkInter\\Second Round Tutorials\\pictures_folder\\{picture_name}.jpg"


        # save the picture to a file
        with open(picture_file, "wb") as writeFile:
...
class App(tk.Tk):
    ...
    def set_picture(self, file_path):
        ...
        # resize the picture
        resized_img = pil_img.resize((self.canvas_width, self.canvas_height), Image.ANTIALIAS)

        self.img = ImageTk.PhotoImage(resized_img)


        # set background image
        self.bg = self.canvas_create_img(0, 0, anchor = tk.NW, image = self.img)
